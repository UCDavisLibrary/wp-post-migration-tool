...
class ExhibitTransform {
  ...
  async run(opts={}) {
    ...
    // if( opts.skipExisting === false ) {
      // console.log('  Exhibit removal is disabled');
    // }
    if ( exists ) return console.log('  Exhibit exists, skipping');
  ...
  async parseContent() {
    ...
    let flexContentList = this.post?.acf?.ucd_pages_flexible_content || [];
    this.flexContentHtml = [];
    for( let item of flexContentList ) {
      this.flexContentHtml.push(await flexContent.transform(item));
    }

    this.flexContentHtml = this.flexContentHtml.join('\n');
  ...
  async embedMedia() {
    ...
    for( let src of sources ) {
      ...
      this.content = this.content
        ...
        .replaceAll('"id":'+sourceMedia.id, '"id":'+sinkMedia.id)        
