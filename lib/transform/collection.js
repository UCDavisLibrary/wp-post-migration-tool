...
// 'wp_user_id'
  ...
  const META_MAP = {
    ...
    'acf.ucdlib_uacollections_subject' : 'subject',
    'link' : 'links',
    'slug' : 'slug',
    'type': 'collectionType',
    'id' : 'fetchedData'
 ...
 class CollectionPost {
    ...
    async parse() {
      ...
      this.data.title = this.row.title ? this.row.title: this.row["title.rendered"];
      this.data.slug = this.slug(this.row) ? this.slug(this.row) : clean(this.data.title);
      ...
      if( !item ) {
        // get subject tags from old site and add to new site, also if manuscript, add the taxonomy
        let tags = await wp.getTags('post', Number(this.row.id));
        ...
        if( tags ) {
          ...
          // if no subject, use taxonomy tags
          if (!this.data.meta.subject) {
            this.data.meta.subject = tags.map(t => t.name);
            this.data.meta.fetchedData.subject = tags.map(t => t.name);
          }

          if (this.data.meta.collectionType === 'manuscript') {
            // add taxonomy for manuscripts
            this.data['collection-subject'] = [];
            for( let tag of tags ) {
              this.data['collection-subject'].push(
                await this.getTaxId('collection-subject', tag.name)
              );
            }
          }
        }
    ...
    fetchedData(data) {
      let res = {
        ...
        "callNumber": this.callNumber(data) ? this.callNumber(data) : "",
    ...
    almaRecordId(data){
      ...
      if(data["recordid"].includes("alma")) {
        res = String(data["recordid"].split('alma')[1]);
      }
      else{
        res = String(data["recordid"]);
      }
      return res
    }
    ...
    callNumber(data) {
      ...
      if( data['acf.ucdlib_manuscript_call_num']) {
        return data['acf.ucdlib_manuscript_call_num'];
      }
      if( data['acf.ucdlib_uacollections_call_num']) {
        return data['acf.ucdlib_uacollections_call_num'];
      }
    }
    ...
    findingAid(data){
      ...
      if(data['ark'])
        return {
          "id": "",
          "linkType": "",
          "linkURL": "https://oac.cdlib.org/findaid/" + data["ark"],
          "linkTitle": "Finding Aid on the Online Archive of California",
          "displayLabel": "",
        }
    }
    ...
    links(data) {
      let res = [];
      if (data['acf.ucdlib_manuscript_moreinfo_url']) res.push({id:'', linkType:'referenceInfo', linkURL: data['acf.ucdlib_manuscript_moreinfo_url'], displayLabel : data['acf.ucdlib_manuscript_moreinfo_name']});
      if (data['acf.ucdlib_manuscript_moreinfo2_url']) res.push({id:'', linkType:'referenceInfo', linkURL: data['acf.ucdlib_manuscript_moreinfo2_url'], displayLabel : data['acf.ucdlib_manuscript_moreinfo2_name']});
      if (data['link'])  res.push({id:'', linkType:'link', linkURL: data['link'], displayLabel : ''});
      if (data['_links.self'])  res.push({id:'', linkType:'self', linkURL: data['_links.self'], displayLabel : ''});
      if (data['_links.collection']) res.push({id:'', linkType:'collection', linkURL: data['_links.collection'], displayLabel : ''});
      if (data['_links.about']) res.push({id:'', linkType:'about', linkURL: data['_links.about'], displayLabel : ''});
      if (data['_links.wp:attachment']) res.push({id:'', linkType:'wp:attachment', linkURL: data['_links.wp:attachment'], displayLabel : ''});
      if (data['_links.wp:term']) res.push({id:'', linkType:'wp:term', linkURL: data['_links.wp:term'], displayLabel : ''});
      if (data['_links.curies']) res.push({id:'', linkType:'curies', linkURL: data['_links.curies'], displayLabel : ''});
      return res;
    }
    ...
    content() {
      ...
      <!-- wp:ucd-theme/special-additional-info /-->`;
