...
class WPApi {
  ...
  getTypeName(type) {
    ...
    if( type === 'collection-subject') return type;
  ...
  async createMedia(filename, mimetype, blob, body) {
    ...
    let resp = await fetch(host+apiPath+'/media', {
      ...
      headers : {
        ...
        'Authorization' : 'Basic '+ Buffer.from(config.sink.username+':'+config.sink.key).toString('base64'),
  ...
  getTags(type, id, source=true) {
    return this.fetchJson('https://www.library.ucdavis.edu/wp-json/wp/v2/tags?'+type+'='+id, source, true, true);
  }
  ...
  async fetchJson(url, source, useCache=true, noAuth) {
    ...
    let resp = await fetch(url, header);
