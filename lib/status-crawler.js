...
class StatusCrawler {
  ...
  async run() {
    ...
    try {
      ...
      // TODO: fetch linkmap from gcs bucket.
      await sitemap.run();
      ...
      let destFileName = new Date().toISOString().replace(/[\.:]/g, '-')+'.json';
      let contents = JSON.stringify({
        timestamp : new Date().toISOString(),
        status: this.urlStatus
      }, '  ', '  ');

      await this.storage.bucket(config.google.storage.bucket).file(destFileName).save(contents);

      destFileName = 'data.json';
